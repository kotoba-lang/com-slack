(ns slack.main
  "Kotodama WASM entrypoint for the Slack clean-room actor (L4) — Clojure port.
  ...
  facts (`slack.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Channel"      :plural "channels"      :id-prefix "slack_cha"
    :fields [:name :isPrivate :topic :memberCount] :required [:name :isPrivate]
    :coerce {:isPrivate :bool :memberCount :int}  :refs {}}
   {:entity "Message"      :plural "messages"      :id-prefix "slack_mes"
    :fields [:channelId :userId :text :ts]        :required [:text :ts]
    :coerce {}                       :refs {:channelId "Channel" :userId "User"}}
   {:entity "User"         :plural "users"         :id-prefix "slack_use"
    :fields [:handle :realName :email :isBot]     :required [:handle :realName]
    :coerce {:isBot :bool}           :refs {}}
   {:entity "Conversation" :plural "conversations" :id-prefix "slack_con"
    :fields [:type :creatorId]                     :required [:type]
    :coerce {}                       :refs {}}
   {:entity "File"         :plural "files"         :id-prefix "slack_fil"
    :fields [:name :mimeType :sizeBytes :uploaderId] :required [:name :mimeType]
    :coerce {:sizeBytes :int}        :refs {}}
   {:entity "Reaction"     :plural "reactions"     :id-prefix "slack_rea"
    :fields [:messageTs :name :count]              :required [:messageTs :name]
    :coerce {:count :int}            :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"slack.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "slack-compat" :tier tier :entities entities} 200])
